...
int main() {
	...
	strncpy( msg, gMes, gMesLen);
	unsigned char *padPos = (gMesLen % 64) == 0 ? NULL:gMes + (gMesLen/64)*64;
	if( padPos ) {
		PadMessage(padPos, lastBlockLen, msg+((num_of_blocks-1)*64));
		//strncpy( msg+((num_of_blocks-1)*64), ptr, 64);
		//ptr = msg+((num_of_blocks-1)*64);
		printf("\n%x %x %x %x %x \n", msg[0], msg[1], msg[2], msg[3], msg[63] );
	}
	int len_in_bits = ((gMesLen/64) + 64)*8;

	sha_256(msg, len_in_bits);
	/*printf("\n\n");
	for( i=0; i<64; i++ ) {
		printf("%s ", byte_to_binary(padPos[i]));
		//printf("%x ", padPos[i]);
	}*/
#else
